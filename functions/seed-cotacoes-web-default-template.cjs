// v4.63.30+ — Seed template Web Link default Cotações (paridade Portal) (modo 'full')
const fs = require('fs');
...
const TEMPLATE_FILE = path.join(__dirname, '..', 'templates', 'seeds', 'cotacoes-web-default-html.html');
const TEMPLATE_NAME = 'PRIMETOUR Cotações Web Link — Default (Full mode)';
...
(async () => {
  ...
  await db.collection('templates').doc(templateId).set({
    name: TEMPLATE_NAME,
    description: 'Template HTML Web Link Cotações modo FULL — substituicao completa de roteiro-view.html. Hero com nome cliente + período, cards dia a dia + hotéis + voos + pricing destacado. JS hooks PRIMETOUR.onDiaClick + onHotelClick wire em clique. Footer com cotação preparada por. Inclui OG meta dinamico pra crawlers WhatsApp/social.',
    ...
    seedSource: 'cotacoes-web-default-html.html v4.63.23',
  });
