...
import { renderDashboards, destroyDashboards } from './pages/dashboards.js?v=20260508r9';
...
import { renderNlPerformance }            from './pages/nlPerformance.js?v=20260508r1';
...
function renderApp(root) {
  ...
  setTimeout(async () => {
    try {
      const { maybeStartWelcomeTour } = await import('./services/tours.js?v=20260512oo4');
...
function setupRouter() {
  ...
  router.register({
    ...
    'office':       async () => { const { renderOffice, destroyOffice } = await import('./pages/office.js?v=20260512o16'); window.__officeDestroy = destroyOffice; await renderOffice(content); },
    ...
    // Front-end exclusivo é o link público dev-hours-view.html (sem auth).
    // Entradas são gerenciadas via Claude no chat (commit workflow).
    'help':         async () => { const { renderHelp } = await import('./pages/help.js?v=20260512oo4'); await renderHelp(content); },
    'ai-hub':       async () => { const { renderAiHub } = await import('./pages/aiHub.js?v=20260512oo4'); await renderAiHub(content); },
    ...
    'templates-library':    async () => {
      const { renderTemplatesLibrary } = await import('./pages/templatesLibrary.js?v=4.63.8');
    ...
    'luxury-travel':        async () => { await renderLuxuryTravel(content); },
    'luxury-travel-admin':  async () => { await renderLuxuryTravelAdmin(content); },
  ...
  store.subscribe('activeWorkspaces', () => {
    ...
    const refreshMap = {
      ...
      'dashboards':() => { destroyDashboards(); import('./pages/dashboards.js?v=20260508r9').then(m => m.renderDashboards(content)); },
  ...
  router.afterNavigation(async (route) => {
    ...
    try {
      const { maybeStartModuleTour } = await import('./services/tours.js?v=20260512oo4');
    ...
    setTimeout(async () => {
      try {
        const { mountAgentsForRoute } = await import('./components/agentTrigger.js?v=20260512oo4');
