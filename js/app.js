...
function mountShell(root) {
  ...
  if (currentUser?.uid) {
    ...
    import('./services/agentScheduler.js?v=20260501cc').then(m => m.startAgentScheduler()).catch(() => {});
  }
}
...
function setupRouter() {
  ...
  router.register({
    ...
    'ai-hub':       async () => { const { renderAiHub } = await import('./pages/aiHub.js?v=20260501cc'); await renderAiHub(content); },
  ...
  router.afterNavigation(async (route) => {
    ...
    setTimeout(async () => {
      try {
        const { mountAgentsForRoute } = await import('./components/agentTrigger.js?v=20260501cc');
