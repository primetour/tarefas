...
function renderApp(root) {
  ...
  setTimeout(async () => {
    try {
      const { maybeStartWelcomeTour } = await import('./services/tours.js?v=20260501gg2');
...
function mountShell(root) {
  ...
  if (currentUser?.uid) {
    ...
    import('./services/agentScheduler.js?v=20260501gg2').then(m => m.startAgentScheduler()).catch(() => {});
  }
}
...
function setupRouter() {
  ...
  router.register({
    ...
    'help':         async () => { const { renderHelp } = await import('./pages/help.js?v=20260501gg2'); await renderHelp(content); },
    'ai-hub':       async () => { const { renderAiHub } = await import('./pages/aiHub.js?v=20260501gg2'); await renderAiHub(content); },
  ...
  router.afterNavigation(async (route) => {
    ...
    try {
      const { maybeStartModuleTour } = await import('./services/tours.js?v=20260501gg2');
    ...
    setTimeout(async () => {
      try {
        const { mountAgentsForRoute } = await import('./components/agentTrigger.js?v=20260501gg2');
