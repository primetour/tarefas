    ...
    if (webResults.length > 0) {
      ...
      try {
        ...
        const cadastrarMsg = `Resultados da busca web:\n${webDataStr}\n${existingInfo}\n`
          ...
          + `- Use ${createAction} para CADA resultado. NÃO use ${wasClippingSearch ? 'create_news' : 'create_clipping'}.\n`
          + `- Ignore links de redes sociais (Instagram, Facebook, LinkedIn) e sites institucionais/homepage.\n`
          + `- Para CADA item preencha: title, description (1-2 frases), sourceUrl, sourceName, publishedAt (YYYY-MM-DD).\n`
          ...
          try {
            ...
            if (webData.length > 0) {
              ...
              const chainMsg = `Resultados da busca web:\n${JSON.stringify(webData.slice(0, 10), null, 1).substring(0, 2500)}\n${existingInfo}\n`
                + `TAREFA OBRIGATÓRIA: Cadastre os resultados relevantes usando ${chainCreateAction}.\n`
