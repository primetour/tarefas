...
export function showBankGuardModal({ bankName, clientName, module, contractNote, onChoosePdf, onForceLink, onCancel }) {
  ...
  document.getElementById('bg-force-link')?.addEventListener('click', () => {
    // v4.63.34+ Renê reportou: "clico em 'gerar link mesmo assim', o sistema
    // não reage". Causa raiz: chamar modal.confirm() de DENTRO do click handler
    // dentro de um modal já aberto (bankGuardModal) criava double-modal stack +
    // race condition com ESC global handler. Simplificado: o primeiro modal já
    // mostra disclaimer FORTE sobre responsabilidade contratual — duplo confirm
    // é UX ruim e desnecessário. Click direto fecha + dispara onForceLink.
    console.log('[bankGuard] força link aprovado pelo user — disparando onForceLink');
    close('link');
  });
