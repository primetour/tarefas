...
> **Última atualização:** v4.63.97 (30/05/2026) · 117 dias de calendário · **1.113,52h** approved · **R$ 167.000,83** · 439 entradas aprovadas
>
> _Fix 30/05/2026 — Cotações capa/contracapa linha sobre o logo_: **v4.63.97** — **+0,47h / +R$ 70,69** (1 release, módulo `roteiros`, bucket `trivial`, multiplicadores `pdf`+`investigation`, AI mult 0.5). Renê: *"existe uma linha meio do lado"* na capa/contracapa do PDF de cotação. As linhas-topo do frame decorativo (template HTML render via Puppeteer) cortavam os logos BTG — capa `.cover-line-top` 40mm→24mm (logo top-anchored ~34mm), contracapa `.closing-line-top` 110mm→88mm (logo centralizado ~102-140mm). Linhas-base inalteradas. Template re-seedado no R2 (novo default ativo, antigo arquivado). Validado por render real (Skia 14MB, 35pgs) + inspeção 150dpi pg1+pg35. Lição: SPA hash-route não reseta `_defaultCotacoesTplCache` (módulo) — só full reload.
>
> _Sprint 30/05/2026 — Auditoria de segurança banking-grade_: **v4.63.94 → v4.63.95** — **+2,83h / +R$ 424,31** (2 releases, módulo `security`, bucket `small`, multiplicadores `security`+`investigation`, AI mult 0.5). **Lote 0 (rules, PROD)**: lock de privilege-escalation no self-create/self-update de `users`; `integrations` read restrito a admin; 3 collections `*_dev` world-open travadas; escritas externas (time_clock_audit/csat_surveys/recurring_task_templates) endurecidas. **Lote 1 (cliente)**: XSS armazenado em csat-response.html (escHtml); normalizeUrl allowlist de esquema (bloqueia javascript:/data:); signOut limpa tokens ms/google. **Lote 2 (Cloud Functions)**: descoberta crítica — as 6 roles têm `isSystem===true`, logo `|| isSystem===true` era bypass total (member/partner passavam) → removido de 5 pontos; getAISecretsStatus deixa de vazar tamanho exato das keys (gate + dica grosseira); getGitHubFile SSRF fechado (gate + allowlist repo/branch/path/download_url); callLLM com tetos server-side (≤50 USD/dia, ≤32768 tokens); renderTemplate cap 2MB; saveDestinationPhoto valida ID. 8 functions deployadas PROD us-central1. Relatório completo em `SECURITY-AUDIT-2026-05-30.md`; lições em CLAUDE.md §17. **Flags pro Renê**: headers HSTS/X-Frame só na migração Cloudflare (Sprint 4); CSP unsafe-inline; getR2UploadUrl token compartilhado; tokens de share adivinháveis.
>
> _Sprint 28/05/2026 — Portal de Dicas (rich text, mapa, tags, hero auto + 3 auditorias)_: **v4.63.34 → v4.63.48** — **+6,00h / +R$ 900,00** (15 releases consecutivas, módulo `portal`, bucket `large`, AI mult 0.5). 5 features novas + 3 ciclos de auditoria. **Features**: rich text leve markdown B/I/U/link/anchor (`js/services/richText.js` + XSS protection 3 camadas + 14 testes Node adversariais); subtítulos livres entre itens em segments; âncoras internas (link entre segmentos da mesma dica); tags por item (vocabulário fixo `DEFAULT_TIP_TAGS` 35+ + custom collection `portal_tip_tags`); mapa interativo Leaflet + markercluster + Nominatim com cache `_geo` real-time onSnapshot; reordenar segmentos ▲▼; mover item entre segmentos ⇄; hero auto via heroTasks; auto-photos PDF; UX identidade (logos áreas com `logoUrlAlt` colorida, 4 blocos com chip dourado + border-top accent + shadow, novos labels). **Hotfixes auditoria**: H1-H8 Agent E2E (subtitle render, XSS href, prompt() nativo, anchor modal current segment, subtítulo vazio); H6 markdown leakage + H2 PPTX subtitle; BC1 snap.exists função vs propriedade (3 sites em portal.js — bug latente semanas); BC2 CSS `var(--brand-gold)10` inválido; B1 XSS bypass parcial proto check ANTES da transformação; B2 email com underscore italic word-boundary. **Performance**: mapa 3x mais rápido (4-5s → 1.5-2s via Promise.all em fetchTipsAggregated + paralelizar loadLeaflet + addLayers batch). **Z-index fix**: `isolation:isolate` no wrapper do mapa pra evitar Leaflet controles z=1010 vazarem sobre dropdowns header (Paleta, Perfil). **Patrimônio** (3 scripts repo): `smoke-test-v4-63-46.cjs`, `prefill-geo-cache.cjs` (Nominatim 1/s idempotente, 19/20 destinos cacheados), `inspect-area-logos.cjs`. **Lições novas CLAUDE.md §16.n-r (5 itens)**: SDK v9 modular `snap.exists` é função (não propriedade); CSS `var()10` é declaração inválida; sprint múltiplas features com Agent paralelo pós-cada-feature (ROI medido); schema 2-logos por área (logoUrl=fundo escuro/logoUrlAlt=fundo claro) — confundir = invisível; z-index libs externas (Leaflet/Chart.js) isoladas via `isolation:isolate` desde primeira integração.
>
> _Sprint maratona 27/05/2026 — Editor cotações + GDS parser + notif safety-net + portal_: **v4.62.26 → v4.62.38** — **+16,87h / +R$ 2.530,13** (13 releases consecutivas). 5 frentes:
> 1. **Parser GDS aéreo/hotel completo** (v4.62.26-29): `parsePNR` (Amadeus/Sabre/Galileo voos), `parseHotelPNR` (HHL/HTL), `parseAirFareGds` (pricing display base+taxas+total+breakdown). UI unificada em 1 botão `Codificar do GDS` que aceita itinerário + tarifa juntos OU separados (v4.62.29). v4.62.34 fixes pós-feedback Renê: IATA→nome literário no card, voo fake "US 3874" eliminado (data DDMMM obrigatória + blacklist NON_AIRPORT_CODES com moedas/paxTypes/taxas/commands), breakdown chips removidos da UI (poluíam), opção "distribuir entre voos" removida (semanticamente falsa pra cotação aérea).
> 2. **Editor Serviços refeito** (v4.62.30-32): campo Oportunidade Salesforce (passo 1 de 2, fetch via API futuro); sub-tabs Aéreo+Hotéis/Valores/Opcionais → **form único card-based** (Renê: "eu não pedi pra juntar em UM form só???"); CSS injetado inline com grid responsivo (eliminou overflow lateral + inputs 90px ilegíveis); botão `Revisar tarifa` reabre modal de distribuição sem precisar colar texto GDS de novo (v4.62.32).
> 3. **Fixes críticos** (v4.62.33, v4.62.35-36): confirm de excluir cotação disparava 5-6× (CLAUDE.md §11.k recidivismo — `container.addEventListener` sem AbortController acumulava 5 listeners por visita); compartilhar tarefa via link no rodapé do modal (deep-link já existia, faltava UI; hierarquia respeitada via Firestore Rules — fetchTasks filtra sem permissão).
> 4. **Auditoria notificações + 2 CFs safety-net** (v4.62.37): user reportou que assignees/observers não recebem notif. Auditoria via Agent achou 5 causas. Diagnose primeiro eliminou `settings/global.notifyTaskAssigned` (estava ligado). 3 fixes críticos: GAP 3 (`bulkUpdateTasks` sem notify), GAP 5+4 (Portal 3 callers + recurringTasksDailyCron sem notify — solução §12.n option 3: 2 CFs novas `onTaskCreated` + `onTaskUpdated` cobrem QUALQUER caller atual ou futuro, idempotentes via query 5min). GAP 1: `task.observing` padronizado em maps.
> 5. **Portal de Dicas: editar/excluir segmentos custom** (v4.62.38): backend existia desde v4.40.18, só faltava UI — ✎/🗑 inline em cada segmento custom (builtin protegido).
>
> **Lições novas CLAUDE.md §15 (5 itens):** diagnose-first em audit (eliminar settings/kill-switch antes de codar), CF reativa onCreate/onUpdate como safety-net §12.n option 3, parser GDS guards anti-falso-positivo (blacklist + campo distintivo obrigatório), 1-botão tolerante > N-botões especializados em UX de import, recidivismo §11.k em SPA sempre vaza listener em delegação.
>
> _Sprint Geography SSOT + Banco audit 26/05/2026 tarde_: **v4.59.0 + v4.59.1** (+6,86h / +R$ 1.029,00). **v4.59.0 SSOT geográfico cross-module:** fundação pra alinhar Banco de Roteiros, Banco de Imagens, Portal de Dicas e Portal de Destinos antes da IA usar Banco como knowledge base. Foundation files hardcoded (`js/data/continents.js` 7 entries UN M.49 + `js/data/countries.js` 196 ISO 3166-1 + 4 UK constituents + aliases) + helper `js/services/geoResolver.js`. Schema extension não-destrutivo em 4 collections (campos novos `countryCode`/`continentCode` + `source`/`reviewStatus` em destinations + `cityAliases` + `envisionLocationId`). Backfill cross-modules 100% match (61 destinations + 236 bank + 190 images + 9 tips, zero unresolved). Adapter Envision popula códigos ISO auto. Risco zero pra outros módulos — apenas ADIÇÃO. **v4.59.1 audit fixes Banco:** auditoria via subagent (5 críticos + 8 médios + 8 polish + 10 risk técnico mapeados). Aplicados: limpa código morto filtro continente, FK cleanup `deleteRoteiroBank` (§13.a aplicado em notifications/ai_usage_logs/tasks), hero auto-resolve PARALELO Promise.allSettled batch 5 (5min → 5-10s), sort dropdown 4 modos, filtro coleção, duplicateRoteiroBank zera envision.id. Doc operacional `docs/ENVISION-SYNC-GUIDE.md` (~300 linhas) + UI hint "Como atualizar via Envision" no header banco (modal explicativo + link guia GitHub). **Lições novas CLAUDE.md §14 (10 itens):** SSOT híbrido hardcoded/Firestore, ISO codes como FK + label canônico, aliases-aware lookup, pseudo-continentes legados precisam mapa explícito, backfill idempotente dry-run+apply, schema extension não-destrutivo, doc operacional no módulo + .md, auditoria por subagent paralelo, Envision SOAP-only, paralelização de trabalho. Validação E2E v4.59.0 confirmou zero quebra cross-module. v4.59.1 validado via curl (MCP cache stale, mas código publicado tem todos os fixes).
>
> _Sprint Analytics 26/05/2026_: **v4.57.53 → v4.57.55** (+1,17h / +R$ 174,38). Auditoria + correção dos módulos de Análise de Dados (IA Hub · Dev Hours View · NL Performance · GA Performance · Dashboards): 20 gaps mapeados (#A1-#A20), 7 acionáveis fechados em 3 releases. Anti-padrões §11.k (5× `confirm()` em aiHub + 2× `alert()` em dev-hours-view → `modal.confirm` + showNotice inline). Listener cleanup (`destroyDashboard` agora invocado em `beforeNavigation`). State reset cross-session (nlPerf `hiddenRows`). Query orderBy timestamp desc + banner de truncamento (aiHub costs/logs). Falsos positivos descartados após inspeção: A9 (timezone analytics.js usa `T00:00:00` que é local, não UTC), A12 (insightDrafts já cap MAX_DRAFTS=20 com quota fallback), A20 (mousemove/mouseup self-cleaning per drag), A4 (insightDraftsDock listeners bound com flag + unmount via hashchange).
>
> _Maratona 25-26/05/2026 — auditoria sistemática de 4 módulos_: **v4.57.28 → v4.57.52** — **+27,62h / +R$ 4.140,38** (25 releases consecutivas). Auditorias completas (com Agent in-the-loop pra descoberta de gaps) em **Tarefas**, **Gerador de Roteiros**, **Portal de Dicas**, **Banco de Imagens** — fechando ~46 gaps confirmados em código (mais ~10 falsos positivos descartados após inspeção). 9 Cloud Functions novas/atualizadas (incluindo `recurringTasksDailyCron`, `scheduledNotificationsCron` com pseudo-user `Sistema PRIMETOUR`, `roteiroBankValidityCron`, `onPortalTipUpdated`, `portalImagesOrphanCleanupCron`, `portalTipsStaleCheckCron`, `deleteR2`). 22+ caminhos de cleanup FK cross-collection. Sprint final fechou pendências legacy #59 (dedupe + helper hierárquico portal_destinations) e #60 (quick-add destinos do banco). Security I1 (R2 token hardcoded em GH Pages público) migrado pra CF com validação E2E completa via Chrome MCP (5 cenários: token leak / upload / delete / 404 blob / CF logs). Lições novas CLAUDE.md §13 (10 itens consolidados: FK cleanup pattern · CF agendada com pseudo-user · conflict detection · anti-double-submit · errorCode classification · permissions object shape · getFunctions app explícito · sandbox TCC · banco como repositório · validação E2E MCP).
>
