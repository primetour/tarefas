...
async function searchWeb(query, sites) {
  ...
  // ══════════════════════════════════════════════════════════════
  // ESTRATÉGIA PRIMÁRIA: Google Custom Search JSON API (confiável)
  // Configurar em Configurações > IA: campos googleSearchApiKey e googleSearchCx
  // Gratuita: 100 buscas/dia — https://programmablesearchengine.google.com/
  // Setup: 1) console.cloud.google.com → habilitar Custom Search API
  //        2) programmablesearchengine.google.com → criar engine (buscar toda web)
  //        3) Copiar API Key e CX para as configurações do sistema
  // ══════════════════════════════════════════════════════════════
  try {
    const { getAIConfig } = await import('./ai.js');
    const cfg = await getAIConfig() || {};
    const googleApiKey = cfg.googleSearchApiKey || '';
    const googleCx = cfg.googleSearchCx || '';
